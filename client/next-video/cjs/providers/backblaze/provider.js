...
async function initS3() {
  ...
  s3 ?? (s3 = new import_client_s3.S3Client({
    ...
    region,
    credentials: {
      accessKeyId: backblazeConfig?.accessKeyId ?? import_node_process.env.BACKBLAZE_ACCESS_KEY_ID ?? "",
      secretAccessKey: backblazeConfig?.secretAccessKey ?? import_node_process.env.BACKBLAZE_SECRET_ACCESS_KEY ?? ""
    }
  }));
  if (!bucketName) {
    try {
      ...
      if (bucket) {
        ...
        import_logger.default.info(import_logger.default.label("Using existing Backblaze bucket:"), bucketName);
      }
    } catch (err) {
      import_logger.default.error("Error listing Backblaze buckets");
      console.error(err);
    }
  }
  if (!bucketName) {
    bucketName = `next-videos-${createId()}`;
    import_logger.default.info(import_logger.default.label("Creating Backblaze bucket:"), bucketName);
    try {
      ...
      import_logger.default.error("Error creating Backblaze bucket");
      console.error(err);
    }
  }
}
...
async function putAsset(filePath, size, stream) {
  ...
  let key;
  try {
    key = await (0, import_provider.createAssetKey)(filePath, "backblaze");
    ...
    import_logger.default.error("Error uploading to Backblaze B2");
    console.error(e);
    return;
  }
